...
void CFileTransferSend::slotStreamStatus(const SAM_Message_Types::RESULT result, const qint32 ID, QString Message)
{
    ...
    if(mStreamID!=ID){
        qCritical()	<<"File\t"<<__FILE__<<"\n"
            <<"Line:\t"<<__LINE__<<"\n"
            <<"Function:\t"<<"CFileTransferSend::slotStreamStatus"<<"\n"
            <<"Message:\t"<<"mStreamID!=ID WTF"<<"\n"
            <<"mStreamID:\t"<<mStreamID<<"\n"
            <<"ID:\t"<<ID<<"\n";
    }
...
void CFileTransferSend::StartFileTransfer(qint64 mFromPos)
{
    ...
    if(mUsingProtocolVersionD==0.1){
        ...
        qWarning()<<"File\t"<<__FILE__<<"\n"
              <<"Line:\t"<<__LINE__<<"\n"
              <<"Function:\t"<<"CFileTransferSend::StartFileTransfer"<<"\n"
              <<"Message:\t"<<"Unsupported Protocolversion:"<<mUsingProtocolVersion<<"\n";
...
void CFileTransferSend::slotCalcAverageTransferSpeed()
{
    ...
    QString speedType;
    int speed;
    int departedtime=(mElapsedTimer.elapsed()/1000);
