        ...
        else if(state != TagText)
        {
            ...
            if(!m_check_space || chars == begin || (chars-1)->isSpace())
            {
                ...
                for( ; it != m_emoticons.constEnd(); ++it )
                {
                    ...
                    {
                        appendEmoticon(result, (*it).second, message.mid(chars - begin, (*it).first.length()));
                        found = true;
                        at_amp = false;
                        chars += length;
                        cur = *chars;
...
void CTextEmotionChanger::appendEmoticon(QString& text, const QString& url, const QStringView& emo) const
{
    int i = 0, last = 0;
    while((i = url.indexOf(QLatin1String("%4"), last)) != -1)
    {
        text += url.mid(last, i - last);
        text += emo;
        last = i + 2;
    }
    text += url.mid(last, url.length() - last);
}
